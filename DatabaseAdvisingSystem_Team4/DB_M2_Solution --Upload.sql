...
--create database Advising_Team_4;
go
Use Advising_Team_4
...
---------------------------------PreqCourse_course-------------------------------------
     ...
     prerequisite_course_id int Foreign key references course(course_id)  on update no action on delete no action,
     course_id int Foreign key references course(course_id) on update no action on delete no action ,
...
DELETE graduation_plan
DELETE course_semester
DELETE Student_phone
DELETE preqCourse_course
DELETE instructor_course
DELETE slot
DELETE student_instructor_course_take
DELETE course
DELETE Student
DELETE instructor
DELETE semester
...
select * from Course inner join Slot on Course.course_id = Slot.course_id   
select * from Slot
select * from Course
select * from PreqCourse_course
exec insertall
...
drop proc createalltables
exec createalltables
exec Procedures_AdminDeleteCourse 8
...
as
 ...
 select * from Student s inner join Payment p on s.student_id=p.student_id inner join Installment i on p.payment_id=i.payment_id 
...
exec Procedures_AdminDeleteSlots 'W22'
...
if @type like '%course%' and @studentah >= @course_hours and @isoffered = 1 and @prerequiste = 1 and Not Exists (select * from Student_Instructor_Course_take where semester_code=@current_semester_code AND student_id=@studentid and course_id=@requestcourse_id)
Begin
set @status = 'Accept' 
...
select * from Student_Instructor_Course_take s where student_id=1
---------------------------------------------------------------------------------
...
select dbo.FN_StudentCheckSMEligibility (2,1)
exec [Procedures_StudentRegisterSecondMakeup] 1,2,'W24'
...
select * from Student_Instructor_Course_take where student_id=1
...
-- Student table
INSERT INTO Student (f_name, l_name, gpa, faculty, email, major,financial_status, password, semester, acquired_hours, assigned_hours, advisor_id)
VALUES
('Michael', 'Johnson', 3.5, 'Engineering', 'michael.j@example.com', 'Computer Science',0, 'michael_pass', 1, 345, 15, 1),
('Sarah', 'Williams', 3.2, 'Business', 'sarah.w@example.com', 'Finance',1, 'sarah_pass',  2, 30, 10, 2),
('Christopher', 'Miller', 3.8, 'Science', 'chris.m@example.com', 'Physics',1, 'chris_pass', 4, 60, 20, 3),
('Emma', 'Taylor', 3.6, 'Arts', 'emma.t@example.com', 'English Literature',1, 'emma_pass', 3, 45, 15, 4),
('James', 'Anderson', 3.4, 'Engineering', 'james.a@example.com', 'Electrical Engineering',0, 'james_pass', 2, 30, 10, 5);
...
('2024-06-19 13:00:00', 'Second_makeup', 2);
-- Exam_Student table
...
-- Payment table
INSERT INTO Payment (payment_id,amount, deadline, n_installments, fund_percentage,status, student_id, semester_code, startdate)
VALUES
(1,20000, '2024-09-1', 12, 50,'Paid', 1, 'w21', '2023-09-01'),
(2,15000, '2023-10-1', 1, 30, 'notPaid',2, 'w22', '2023-09-01'),
(3,18000, '2023-11-15', 4, 60, 'notPaid',3, 'w23', '2023-07-15'),
(4,25000, '2024-09-10', 8, 50, 'notPaid',4, 'w22', '2024-01-10'),
(5,20000, '2024-2-10', 4, 30, 'notPaid',5, 'w24', '2023-10-10');


INSERT INTO Installment(payment_id,startdate, deadline, amount,status)
VALUES
(1, '2021-09-1', '2022-09-1', 50,'Paid');
