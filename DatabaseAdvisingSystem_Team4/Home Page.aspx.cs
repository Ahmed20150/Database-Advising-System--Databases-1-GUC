...
namespace GUI
{
    public partial class Home_Page : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
			...
			using (SqlConnection connaf = new SqlConnection(connstr))
			{
				SqlCommand cmd = new SqlCommand($"SELECT dbo.FN_StudentUpcoming_installment({Login.idmoment})", connaf);
				...
				try
				{
					...
					if (result != null && result != DBNull.Value)
					{
						...
						Response.Write($"The deadline of the upcoming installment is: {dt.ToString()}");
					}
					else
					{
						Response.Write("No upcoming installment found.");
					}
				}
				catch (Exception ex)
				{
        ...
        {
            Response.Redirect("view course taught by specific instructor.aspx");
        }
		...
		protected void CS(object sender, EventArgs e)
        {
			string connstr = WebConfigurationManager.ConnectionStrings["Advising_Team_4"].ToString();
			SqlConnection conn = new SqlConnection(connstr);
			SqlCommand gettable = new SqlCommand("select * from Courses_Slots_Instructor", conn);
